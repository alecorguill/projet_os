#!/bin/bash
echo \#Fichier de comparaison de temps d execution sur le test 31 > graph.data
for((i=0;i<250;i+=5))
do
start=`date +%s%N`
start=$((start/1000))
...
gnuplot -e "set title 'Comparaison temps execution sur le test 31';
